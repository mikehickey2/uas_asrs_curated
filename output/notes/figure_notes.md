...
- **Denominators**: N = 50 total reports. Detector field available for 47 reports; phase available for 47 reports. 'Unknown' indicates missing or not reported values.
...
- **Denominators**: N = 50 reports with complete data for all markers (n_available = 50). Wilson 95% confidence intervals reflect sampling uncertainty.
...
- **Denominators**: N = 50 reports. Anomaly field present in 50 reports; contributing factors field present in 50 reports. Counts are report-level (each tag counted once per report regardless of repetition).
